...
@dataclass
class SessionListItemRenderable:
    chat: ChatData
    config: LaunchConfig
...
class SessionListItem(Option):
    def __init__(self, chat: ChatData, config: LaunchConfig) -> None:
        """
        Args:
            chat: The chat/session associated with this option.
        """
        super().__init__(SessionListItemRenderable(chat, config))
        self.chat = chat
        self.config = config


class SessionsWidget(OptionList):
    BINDINGS = [
        Binding(
            "escape",
        ...
        Binding(
            ...
            "Archive session",
            key_display="a",
            tooltip="Archive the highlighted session"
    ...
    async def post_session_opened(self, event: OptionList.OptionSelected) -> None:
        assert isinstance(event.option, SessionListItem)
        chat = event.option.chat
        await self.reload_and_refresh()
        self.post_message(SessionsWidget.SessionOpened(chat=chat))
    ...
    async def load_session_list_items(self) -> list[SessionListItem]:
        sessions = await self.load_sessions()
        return [SessionListItem(session, self.app.launch_config) for session in sessions]

    async def load_sessions(self) -> list[ChatData]:
        all_chats = await ChatsManager.all_chats()
        return all_chats

    async def action_archive_session(self) -> None:
        if self.highlighted is None:
            return

        item = cast(SessionListItem, self.get_option_at_index(self.highlighted))
        ...
        self.border_subtitle = self.get_border_subtitle()
        self.app.notify(
            item.chat.title or f"Session [b]{chat_id!r}[/] archived.",
            title="Session archived",
        )
        self.refresh()

    def get_border_title(self) -> str:
        return f"Sessions ({len(self.options)})"
    ...
    def create_session(self, chat_data: ChatData) -> None:
        new_session_list_item = SessionListItem(chat_data, self.app.launch_config)
        log.debug(f"Creating new session {new_session_list_item!r}")

        option_list = self.query_one(OptionList)
        self.options = [
            new_session_list_item,
            *self.options,
        ]
